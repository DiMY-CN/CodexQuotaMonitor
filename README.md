...
这是一个个人自用的 Windows 原生桌面版本，用 WPF 显示 Codex 的 5 小时额度、一周额度和最近全局上下文使用情况。窗口默认无边框、置顶、贴靠任务栏左侧，并提供托盘菜单。
...
- 三个圆形向量 gauge 使用绿色、黄色、红色提示余量状态；阈值可在 `settings.json` 中调整。
...
![原生版右键菜单](assets/context-menu.png)
...
上下文查询读取 Codex home 目录下的 `logs_2.sqlite` 和 `models_cache.json`。程序会扫描最近 20 条 `response.completed` 记录，跳过格式异常的行，并用模型缓存中的窗口信息估算 CTX 剩余比例。CTX 来源固定标注为 `latest global`，表示最近全局完成的会话，不保证等于当前前台会话。
...
- Codex 本地日志可能包含会话元数据或敏感上下文；公开仓库不应提交 `logs/`、`settings.json`、`publish/` 或任何本机数据文件。
- 读取失败时 GUI 会尽量保留最后一次有效读数，并在标题/提示中标记错误或旧数据状态。

## 分支关系

当前分支是 C# WPF 原生 Windows 版本，也是更适合日常常驻使用的版本。仓库中的 `python` 分支保留 Python/Tk 版本：它不提供原生 EXE，也不自动管理 Python 环境，需要用户用自己选择的 Python 运行 `python codex_quota_float.py`。Python 版更方便阅读和修改，原生版的窗口、托盘和任务栏集成更自然。
...
设置优先级为：命令行参数 > `settings.json` > 内置默认值。右键菜单修改 quota/context interval 后会写回本地 `settings.json`。
